...
function getFacadeZoneLayout(floorCount) {
  ...
  return zones.reverse().map((zone, zoneIndex, allZones) => {
...
function getFloorClipPath(zone) {
  ...
  const normalizedIndex =
    floorCount <= 1 ? 0 : (zone.index ?? 0) / (floorCount - 1);
...
function toAbsoluteFloorCoords(image, pointsPct, closeCycle = false) {
  ...
  const exportPoints =
    closeCycle && pointsPct.length >= 3
      ? [...pointsPct, pointsPct[0]]
      : pointsPct;
...
export function ProjectFloorElevationBlock({ project, embedded = false }) {
  ...
  const maxFloorNumber = floors.length
    ? Math.max(...floors.map((floor) => floor.number))
    : 0;
  const minFloorNumber = floors.length
    ? Math.min(...floors.map((floor) => floor.number))
    : 0;
  ...
  const planTracePointsPct =
    currentPlanTracePointsByApartment[planTraceApartmentIndex] || [];
  ...
  const planApartmentZones = panelApartmentLabels.map(
    (_label, apartmentIndex) =>
      pointsPctToPolygonString(
        currentPlanTracePointsByApartment[apartmentIndex],
      ) ||
      pointsPctToPolygonString(
        apartmentPolygonsForFloor[apartmentIndex]?.coordsPct,
      ) ||
      null,
  );
  const planApartmentColors = panelApartmentLabels.map(
    (_label, apartmentIndex) =>
  ...
  useEffect(() => {
    ...
    const scrollTarget = embedded ? window : page;
    scrollTarget.addEventListener("scroll", reposition, { passive: true });
    ...
    <>
      {wrapElevation(
        <>
          ...
          <div
            className={`floor-elevation${isTiranaSignature ? " is-tirana" : ""}`}
          >
            <div
              className="controls-outer"
            ...
            >
              ...
              >
                ...
                <div className="current-holder">
                  <div
                    className="current-floor-number"
                    data-floor={activeFloorNumber ?? ""}
                  >
                    {activeFloorNumber ?? "—"}
                  </div>
                  <div
                    className="current-invisible"
                    data-show={activeFloorNumber ?? ""}
                  />
                </div>
                <div className="controls-dots">
            ...
            >
              <img
                ...
                src={project.elevationImageSrc || "/assets/building5.jpg"}
              ...
              <canvas
                ...
                className="floor-trace-preview-canvas"
              />
              {traceEnabled &&
                savedTraceIndexes.length > 0 &&
                (() => {
              ...
              >
                {floors.map((floor, index) => {
                  const zone =
                    facadeZones[index] ?? facadeZones[facadeZones.length - 1];
                  const absoluteGeometry = getAbsoluteFloorGeometry(
                    project.id,
                    floor.number - 1,
                  );
                  const clipPoints =
                    absoluteGeometry?.clipPoints || getFloorClipPath(zone);
          ...
          <div className="floor-panel-shell">
            ...
            <div className="floor-panel-main">
              <div className="floor-panel-head">
                ...
                <div className="floor-panel-zone-pill">
                  {floorPanelZonePillLabel}
                </div>
              </div>
                ...
                <div className="floor-panel-plan-column">
                  <div className="floor-panel-plan-wrap">
                    <div
                      ...
                      onPointerMove={(event) => {
                        ...
                        const next = getPctFromImageEvent(
                          event,
                      ...
                      onPointerDown={(event) => {
                        ...
                        if (
                          shouldClosePolygon(next, planTracePointsPct, rect)
                        ) {
                    ...
                    >
                      ...
                      {planApartmentZones.some(Boolean) && (
                        <svg
                          ...
                          aria-hidden="true"
                          style={{
                            pointerEvents: planTraceEnabled ? "none" : "auto",
                          }}
                        >
                  ...
                  {SHOW_FLOOR_PLAN_TRACE_TOOL ? (
                    <div className="floor-plan-trace-tool">
                        ...
                        <button
                          ...
                          onClick={() => {
                            setPlanTracePointsByFloor((prev) => {
                              ...
                              const points = [
                                ...(floorPoints[planTraceApartmentIndex] || []),
                              ];
                              points.pop();
                              floorPoints[planTraceApartmentIndex] = points;
                              return {
                                ...prev,
                                [panelFloorDisplayNumber]: floorPoints,
                              };
                            });
                            setPlanTraceClosedByFloor((prev) => ({
                              ...prev,
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            const payload = {
                              ...
                              polygon:
                                pointsPctToPolygonString(planTracePointsPct),
                              coords: toAbsoluteFloorCoords(
                                image,
                                planTracePointsPct,
                                planTraceIsClosed,
                      ...
                      <p className="floor-trace-help">
                        Enable trace, click apartment points clockwise on the
                        plan, then click the first point to close and copy
                        coords.
                      </p>
                      {planTraceStatus ? (
